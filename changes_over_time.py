...
def calc_distance_over_time(vectors_over_time, word1, word2, distype='norm', vocabd=None, word1lims=[50, 1e25],
                            word2lims=[50, 1e25]):
    ret = []
...
def calc_distance_over_time_averagevectorsfirst(vectors_over_time, words_to_average_1, words_to_average_2,
                                                distype='norm', vocabd=None, word1lims=[50, 1e25],
                                                word2lims=[50, 1e25]):
    retbothaveraged = []
    retfirstaveraged = []
    retsecondaveraged = []
    ...
    for en, vectors in enumerate(vectors_over_time):
        ...
        if len(validwords1) == 0 or len(validwords2) == 0:
            ...
            retfirstaveraged.append(np.mean(
                [calc_distance_between_vectors(average_vector_1, vectors[word], distype) for word in validwords2]))
            retsecondaveraged.append(np.mean(
                [calc_distance_between_vectors(vectors[word], average_vector_2, distype) for word in validwords1]))
...
def single_set_distances_to_single_set(vectors_mult, targetset, otherset, vocabd, word1lims=[50, 1e25],
                                       word2lims=[50, 1e25]):
    '''
    ...
    for word in targetset:
        for word2 in otherset:
            dists = calc_distance_over_time(vectors_mult, word, word2, vocabd=vocabd, word1lims=word1lims,
                                            word2lims=word2lims)
            dists_cossim = calc_distance_over_time(vectors_mult, word, word2, distype='cossim', vocabd=vocabd,
                                                   word1lims=word1lims, word2lims=word2lims)
            # print(dists)
    ...
    averageboth, averagefirst, averagesecond = calc_distance_over_time_averagevectorsfirst(vectors_mult, targetset,
                                                                                           otherset, distype='norm',
                                                                                           vocabd=vocabd,
                                                                                           word1lims=word1lims,
                                                                                           word2lims=word2lims)

    averageboth_cossim, averagefirst_cossim, averagesecond_cossim = calc_distance_over_time_averagevectorsfirst(
        vectors_mult, targetset, otherset, distype='cossim', vocabd=vocabd, word1lims=word1lims, word2lims=word2lims)

    return [toset, toset_cossim, averageboth, averagefirst, averagesecond, averageboth_cossim, averagefirst_cossim,
            averagesecond_cossim]
...
def main(filenames, label, csvname=None, neutral_lists=[], group_lists=['male_pairs', 'female_pairs'],
         do_individual_group_words=False, do_individual_neutral_words=False, do_cross_individual=False):
    vocabs = [fi.replace('vectors/normalized_clean/vectors', 'vectors/normalized_clean/vocab/vocab') for fi in
              filenames]
                ...
                with open('data/' + grouplist + '.txt', 'r') as f2:
                    ...
                    if do_individual_neutral_words:
                        for word in neutwords:
                            ...
                            dloc_neutral[word][grouplist] = single_set_distances_to_single_set(vectors_over_time,
                                                                                               [word], groupwords,
                                                                                               vocabd)
                    if do_individual_group_words:
                        ...
                        for word in grouplist:
                            ...
                            d_group_so_far[word][neut] = single_set_distances_to_single_set(vectors_over_time,
                                                                                            neutwords, [word], vocabd)
                    ...
                    if do_cross_individual:
                        ...
                        for word in groupwords:
                            ...
                            for neutword in neutwords:
                                d_cross[word][neutword] = single_set_distances_to_single_set(vectors_over_time,
                                                                                             [neutword], [word], vocabd)
...
filename_map = {'nyt': filenames_nyt, 'sgns': filenames_sgns, 'svd': filenames_svd, 'google': filenames_google,
                'wikipedia': filenames_wikipedia, 'commoncrawlglove': filenames_commoncrawl}
...
if __name__ == "__main__":
    ...
    with open(param_filename, 'r') as f:
        ...
        for row in reader:
            ...
            main(filename_map[label], label=label, csvname=row['csvname'], neutral_lists=neutral_lists,
                 group_lists=group_lists, do_individual_neutral_words=do_individual_neutral_words,
